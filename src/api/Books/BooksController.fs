...
module Controller = 
  let indexAction (ctx: HttpContext) = task {
    ...
    let! (result: Result<seq<Book>,exn>) = Database.getAll cnf.connectionString

    match result with
    | Ok result -> return! Controller.json ctx result
  ...
  let createAction (ctx: HttpContext) = task {
    ...
    let! book = Controller.getJson ctx
    let bookDto = 
      book
      |> (fun obj-> 
          {
    ...
    let! result = Database.insert cnf.connectionString bookDto
